...
class Config(BaseSettings):
    ...
    language_recognize: str = Field(default="ru-RU", alias="LANGUAGE_RECOGNIZE")
    language_tts: str = Field(default="ru", alias="LANGUAGE_TTS")
    ...
    not_found_voice_path: str = Field(
        default=os.path.join(base_dir, "voice_files", "system", "not_found_voice.wav"),
    ...
    rabbitmq_web_interface_port: int = Field(default=15672, alias="RABBITMQ_WEB_INTERFACE_PORT")

    def get_rabbitmq_url(self) -> str:
        return f"amqp://{self.rabbitmq_user}:{self.rabbitmq_pass}@{self.rabbitmq_host}/"

    declare_queue_name: str = Field(default="voice_assistant_request", alias="RABBITMQ_INCOMING_QUEUE_NAME")
    ...
    # MovieService
    movies_service_host: str = Field(default="movies_service", alias="MOVIES_SERVICE_HOST")
    movies_service_port: int = Field(default=8000, alias="MOVIES_SERVICE_PORT")
    movies_service_schema: str = Field(default="http", alias="MOVIES_SERVICE_SCHEMA")

    def get_movies_service_url(self) -> str:
        return f"{self.movies_service_schema}://{self.movies_service_host}:{self.movies_service_port}"
