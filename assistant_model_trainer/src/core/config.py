...
class NLPConfig(BaseSettings):
    ...
    pattern_path: str = Field(
        default=os.path.join(base_dir, "train_patterns.json"),
    )
    ner_labels: list[str] = ["MOVIE"]
    is_resume_training: bool = Field(
        default=True,
        alias="NLP_IS_RESUME_TRAINING",
    )

    ner_task_trigger: str = Field(
        default="interval",
        alias="NER_TASK_TRIGGER",
    )
    ner_task_interval_days: int = Field(
        default=1,
        alias="NER_TASK_INTERVAL_DAYS",
    )
    ...
    def get_load_base_model(self, train_type: bool = True) -> str:
        ...
        - Выбирается обученная модель 'ru_core_news_sm' и
        на ее основе происходит дообучение модели.
        - Выбирается ранее обученная модель 'movie_ner_model' и
        проходит дообучение (additional).

        @type train_type: bool
        @param train_type: Флаг - обучение модели с нуля
        (на основе 'ru_core_news_sm') или дообучение существующей.
        @rtype: str
        @return:
        """
        if train_type:
